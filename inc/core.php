...
function renderAlbum($data)
{
    ...
    foreach ($data['album'] as $hash) {
        $content .= '<a href="' . PATH . $filters . $hash . '"><img class="picture" src="' . PATH . $size . $forcesize .
                    $filters . $hash . '" /></a>';
    }
...
function renderImage($data)
{
    ...
    if (file_exists($cachepath)) {
        ...
        if (MAX_RESIZED_IMAGES > -1 && $pm->countResizedImages($hash) >
                                       MAX_RESIZED_IMAGES) { //if the number of max resized images is reached, just show the real one
...
{
    $image = new \App\Transformers\Image();

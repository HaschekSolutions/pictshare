...
_maxUploadSize() {
	...
	sed -i "/post_max_size/c\post_max_size=${MAX_UPLOAD_SIZE}M" /etc/php84/php.ini
	sed -i "/upload_max_filesize/c\upload_max_filesize=${MAX_UPLOAD_SIZE}M" /etc/php84/php.ini

    # set error reporting no notices, no warnings
    sed -i "/^error_reporting/c\error_reporting = E_ALL & ~E_DEPRECATED & ~E_STRICT & ~E_WARNING & ~E_NOTICE" /etc/php84/php.ini
    ...
    sed -i -e "s/128M/${MAX_RAM}M/g" /etc/php84/php.ini
	sed -i "/memory_limit/c\memory_limit=${MAX_RAM}M" /etc/php84/php.ini
}

_filePermissions() {
    chown -R nginx:nginx /var/www/tmp /var/www/data
    chmod 1777 /var/www/tmp /var/www/data
    touch data/sha1.csv

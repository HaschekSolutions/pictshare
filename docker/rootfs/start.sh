...
if [[ ${REDIS_CACHING:=true} == true ]]; then
    ...
    # 5-minute initial delay so it's not racing container/Redis startup, then it
    # ticks daily from there (restart-heavy deployments would otherwise never
    # flush, since the old loop slept 24h before its first run).
    (sleep 300; while true; do php /app/public/tools/cron.php flushviews; sleep 86400; done) &
fi
